...
## Session Notes
### 2026-05-28
- Done: Added Prisma schema for Blog module, created migration
- Next: Implement tRPC router for posts CRUD
```

### decisions.md — When choosing between approaches:

```markdown
## ADR-001: Use Dual Auth Strategy (NextAuth + JWT)

### Context
Need to support both web (cookie) and mobile (token) clients.

### Options
1. NextAuth only — simpler, but mobile can't use cookies natively
2. JWT only — works everywhere, but lose SSR session benefits
3. Dual auth (NextAuth + JWT) — best of both worlds, more complexity

### Decision
Dual auth strategy, matching Cal.diy's `ApiAuthStrategy` pattern.

### Consequences
- NestJS needs multi-strategy Passport guard
- Token rotation logic for refresh tokens
- Shared `AuthService` in packages/features for both layers
```
